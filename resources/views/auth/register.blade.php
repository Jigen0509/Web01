<x-guest-layout>
    <h2 class="text-xl sm:text-2xl font-bold text-gray-800 mb-4 sm:mb-6 text-center" style="letter-spacing: 0.05em;">新規登録</h2>

    <!-- Validation Errors -->
    @if ($errors->any())
    <div class="mb-3 sm:mb-4 p-3 sm:p-4 bg-red-50 border-l-4 border-red-500 rounded-md">
        <div class="flex">
            ...
            <div class="ml-2 sm:ml-3">
                <h3 class="text-xs sm:text-sm font-medium text-red-800">入力内容にエラーがあります</h3>
    ...
    <form method="POST" action="{{ secure_url(route('register', [], false)) }}" class="space-y-4 sm:space-y-5">
        @csrf

        <!-- Name -->
        <div>
            <label for="name" class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2" style="letter-spacing: 0.05em;">
                {{ __('Name') }}
            </label>
            <input
                id="name"
        ...
        <!-- Confirm Password -->
        <div>
            <label for="password_confirmation" class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2" style="letter-spacing: 0.05em;">
                {{ __('Confirm Password') }}
            </label>
            <input
        ...
        <div class="text-center mt-3 sm:mt-4">
            <a class="text-xs sm:text-sm text-gray-600 hover:text-green-600 underline transition-colors" href="{{ route('login') }}" style="letter-spacing: 0.03em;">
